...
export default function LanguageSwitcher() {
  ...
  // Opens modal on initial load and every page refresh
  useEffect(() => {
  ...
  const handleLanguageChange = (newLocale: string) => {
    // Immediately close modal upon selecting any language
    setOpen(false);
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-xs w-full p-6 rounded-2xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-center mb-4 text-sm text-gray-500">
          ...
          <div className="flex flex-col gap-3 items-center">
            {LANGUAGES.map((lang) => (
              <button
                ...
                disabled={isPending}
                className={`w-28 h-9 rounded-xl font-bold border-2 text-base flex items-center justify-center transition-all duration-200 cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-[#000000]/55
                  ${
                    locale === lang.code
